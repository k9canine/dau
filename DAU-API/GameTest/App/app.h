...
#define APP_NATIVE_TO_VIRTUAL_COORDS(_x_, _y_)       \
	_x_ = ((_x_ + 1.0f) * APP_VIRTUAL_WIDTH) / 2.0f; \
	_y_ = ((_y_ + 1.0f) * APP_VIRTUAL_HEIGHT) / 2.0f;
...
namespace App
{
	...
	//-------------------------------------------------------------------------------------------
	void Print(float x, float y, std::string text, float r = 1.0f, float g = 1.0f, float b = 1.0f, void *font = GLUT_BITMAP_HELVETICA_18);
